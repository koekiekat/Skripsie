...
def select_templates_interactive(wav_path, segments, f_s, start_t, end_t, labels,
    ...
    def show_next():
        ...
        fig = plt.gcf()          # grab whatever got drawn, regardless of return value
        plt.ion()
...
def extract_background_segments_interactive(wav_path, text_path, n_segments,
    ...
    def show_next():
        ...
        try:
            ...
            plot_spectrogram(f_sig, t_sig, Zxx_sig, fsn)
            fig = plt.gcf()  # works whether or not plot_spectrogram returns a Figure
            with out:
                display(fig)
            ...
            plt.ion()  # ALWAYS restore interactive mode, even if something above fails
        update_status()
...
def review_saved_calls(json_path, fs_new=1000, start_idx=0):
    ...
    def show_current():
        ...
        if state["pos"] >= len(order):
            ...
            btn_stop.disabled = True   # guard against the out-of-range click
            return
